<?PHp 
function gl_insert_color($nombre,$hexa){
		global $conn;
			$queEmp = "INSERT INTO aux_color (nombre
						,codigo_hex,fecha_entrada)VALUES('$nombre','$hexa',now())
					";
...
function gl_get_color($val,$type='')
	{
	...
	$i=0;
		while ($a_resp2 = mysql_fetch_array($a_resp)) {
				...
				$a_vect[$i]['id_color']				=$a_resp2['id'];
				$a_vect[$i]['nombre_color']			=$a_resp2['nombre'];
				$a_vect[$i]['codigo_hex']			=$a_resp2['codigo_hex'];
